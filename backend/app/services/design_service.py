...
from ..schemas.design import DesignCreate, DesignUpdate, RevisionCreate, Industry
...
def get_design_with_versions(db: Session, design_id: str) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "published_to_library": design.published_to_library,
...
def publish_design_to_library(
    ...
    # Soft requirement: design must have at least one completed version, otherwise
    # the library would show empty thumbnails.
    has_image = (
        db.query(DesignVersion)
        .filter(
            DesignVersion.design_id == design_id,
...
def unpublish_design_from_library(
    db: Session,
    design_id: str,
) -> Design:
    """Remove a design from the library. Clears all library_* fields."""
    design = db.query(Design).filter(Design.id == design_id).first()
    if not design:
        raise ValueError("Design not found")

    design.published_to_library = False
    design.library_industry = None
    design.library_published_at = None
    design.library_published_by_id = None
    design.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(design)
    return design
...
def list_library_designs(
    db: Session,
    industry: Optional[str] = None,
    skip: int = 0,
    limit: int = 100,
) -> List[Dict[str, Any]]:
    """List all published library designs, optionally filtered by industry.

    Returns lightweight cards — no full version/chat history, just enough to
    render a thumbnail + remix button.
    """
    ...
    # Eager-resolve publisher names + thumbnail paths in one pass.
    publisher_ids = list({d.library_published_by_id for d in designs if d.library_published_by_id})
    publisher_name_by_id = {}
    if publisher_ids:
        for u in db.query(User).filter(User.id.in_(publisher_ids)).all():
            publisher_name_by_id[u.id] = u.name or u.email

    results = []
    for design in designs:
        # Prefer the selected version, then the latest completed.
        thumbnail_path = None
        if design.selected_version_id:
            sv = db.query(DesignVersion).filter(
                DesignVersion.id == design.selected_version_id,
                DesignVersion.generation_status == "completed",
            ).first()
            if sv and sv.image_path:
                thumbnail_path = sv.image_path
        if not thumbnail_path:
            ...
            if latest:
                thumbnail_path = latest.image_path
...
def get_library_design_remix_data(
    db: Session,
    design_id: str,
) -> Optional[Dict[str, Any]]:
    """Return the spec data needed to prefill the AI Designer for a remix.

    Includes the path to the selected/latest image so the frontend can
    pass it as reference_image_path in the new design create call.
    """
    design = db.query(Design).filter(
        Design.id == design_id,
        Design.published_to_library == True,  # noqa: E712
    ).first()
    if not design:
        return None

    # Pick reference image (selected version → latest completed)
    reference_path = None
    if design.selected_version_id:
        sv = db.query(DesignVersion).filter(
            DesignVersion.id == design.selected_version_id,
    ...
    if not reference_path:
        ...
        if latest:
            reference_path = latest.image_path
