...
import { useCustomDesigns, useDeleteCustomDesign } from '../hooks/useCustomDesigns';
...
import type { CustomDesignListItem, ApprovalStatus } from '../types/api';
...
export function CustomDesignDashboard() {
  ...
  const deleteDesign = useDeleteCustomDesign();
        ...
        {isLoading ? (
          ...
          <div className="card text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-100 mb-2">
              {search || statusFilter ? 'No designs found' : 'No custom designs yet'}
            </h3>
            <p className="text-gray-400 mb-4">
              {search || statusFilter
                ? 'Try adjusting your search or filters'
                : 'Create your first custom design with specific logo placements'}
            </p>
            {!search && !statusFilter && (
              <Link to="/custom-design-builder/new">
                <Button>
      ...
      <div className="aspect-square bg-gray-800 -mx-6 -mt-6 mb-4 relative overflow-hidden">
        {design.latest_image_path ? (
          <img
            ...
            alt={design.design_name || `Custom Design #${design.design_number}`}
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
          <Link to={`/custom-design-builder/design/${design.id}`}>
        ...
        <div className="absolute bottom-2 left-2 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 bg-purple-600/90 text-white">
          <Layers className="w-3 h-3" />
          {design.location_logos.length} location{design.location_logos.length !== 1 ? 's' : ''}
        </div>
      </div>
