...
import type { QuoteOptions, DomesticQuoteResponse, OverseasQuoteResponse, PriceBreak } from '../../api/quotes';
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Wearer's Right</label>
            <select
...
// Domestic Results Component — multi-tier table (matches QuoteEstimator exactly)
function DomesticResults({ result }: { result: DomesticQuoteResponse }) {
  ...
  if (isValidDecoration(result.left_decoration)) {
    componentRows.push({ label: "Wearer's Left", sublabel: result.left_decoration!, key: 'left_decoration_price' });
  }
  if (isValidDecoration(result.right_decoration)) {
    componentRows.push({ label: "Wearer's Right", sublabel: result.right_decoration!, key: 'right_decoration_price' });
  }
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="bg-gray-700/50">
              <td className="py-2 px-2 text-gray-100 font-semibold">Per-Piece Total</td>
              {breaks.map(pb => (
